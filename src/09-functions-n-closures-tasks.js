...
function getPolynom(...args) {
  return (x) => {
    ...
    return [...args]
      .reverse()
      .reduce((acc, val, i) => (acc + (val * (x ** i))), 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argumentsArr = [];
    args.forEach((item) => argumentsArr.push(JSON.stringify(item)));

    logFunc(`${func.name}(${argumentsArr}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argumentsArr}) ends`);
